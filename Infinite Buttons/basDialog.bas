...
Public Function DialogColor(hWnd As Long, c As Control) As Boolean
    ...
    On Error Resume Next
      ...
      Dim x As Long, CS As COLORSTRUC, CustColor(16) As Long
        ...
        CS.Flags = CC_RGBINIT 'CC_SOLIDCOLOR
...
Public Function DialogFont(hWnd As Long, c As Control) As Boolean
    ...
    On Error Resume Next
      ...
      Dim LF As LOGFONT, FS As FONTSTRUC, lLogFontAddress As Long, lMemHandle As Long
        'If c.Font.Bold Then LF.lfWeight = FW_BOLD':( --> replaced by:
        If c.Font.Bold Then
            LF.lfWeight = FW_BOLD
        End If
        'If c.Font.Italic = True Then LF.lfItalic = 1':( --> replaced by:
        If c.Font.Italic = True Then
            LF.lfItalic = 1
        End If
        'If c.Font.Underline = True Then LF.lfUnderline = 1':( --> replaced by:
...
Public Function DialogPrint(hWnd As Long, bPages As Boolean, Flags As Long) As PRINTPROPS
    ...
    On Error Resume Next
      ...
      Dim DM As DEVMODE, PD As PRINTDLGSTRUC, lpDM As Long, wNull As Integer, szDevName As String
        ...
        PD.nToPage = 0
        PD.nMinPage = 0
        'If bPages Then PD.nMaxPage = bPages - 1':( --> replaced by:
        If bPages Then
        ...
        If PrintDlg(PD) Then
            ...
            'If PD.Flags And PD_PRINTTOFILE Then DialogPrint.File = True Else DialogPrint.File = False':( --> replaced by:
            If PD.Flags And PD_PRINTTOFILE Then
                DialogPrint.File = True
              Else
                DialogPrint.File = False
            End If
            'If PD.Flags And PD_COLLATE Then DialogPrint.Collate = True Else DialogPrint.Collate = False':( --> replaced by:
            If PD.Flags And PD_COLLATE Then
                DialogPrint.Collate = True
              Else
                DialogPrint.Collate = False
            End If
...
':) Ulli's VB Code Formatter V2.23.12 (2007-Dec-31 14:18)  Decl: 435  Code: 466  Total: 901 Lines
':) CommentOnly: 180 (20%)  Commented: 10 (1.1%)  Empty: 114 (12.7%)  Max Logic Depth: 3
